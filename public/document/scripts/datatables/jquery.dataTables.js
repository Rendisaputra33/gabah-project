    ...
    /** @lends <global> */ function ($) {
      ...
      function _fnBuildAjax(oSettings, data, fn) {
        ...
        if (oSettings.fnServerData) {
          ...
          oSettings.fnServerData.call(oSettings.oInstance, oSettings.sAjaxSource, data, fn, oSettings);
          ...
          oSettings.jqXHR = oSettings.ajax.call(oSettings.oInstance, data, fn, oSettings);
        } else {
      ...
      function _fnAjaxParameters(oSettings) {
        ...
        param('iDisplayLength', oSettings.oFeatures.bPaginate !== false ? oSettings._iDisplayLength : -1);
      ...
      function _fnInfoMacros(oSettings, str) {
        ...
        // internally
        var iStart = oSettings.oScroll.bInfinite ? 1 : oSettings._iDisplayStart + 1,
      ...
      function _fnSort(oSettings, bApplyClasses) {
        var i,
          ...
          oSort,
          iFormatters = 0,
          aaNestedSort = oSettings.aaSortingFixed !== null ? oSettings.aaSortingFixed.concat(oSettings.aaSorting) : oSettings.aaSorting.slice();
        ...
        if (!oSettings.oFeatures.bServerSide && aSort.length !== 0) {
          ...
          for (j = 0; j < aSort.length; j++) {
            ...
            for (i = 0, iLen = aoData.length; i < iLen; i++) {
              ...
              aoData[i]._aSortData[oSort.col] = oSort.format ? oSort.format(data) : data;
            }
          }
        ...
        for (i = 0, iLen = oSettings.aoColumns.length; i < iLen; i++) {
          ...
          if (aoColumns[i].bSortable) {
            if (aSort.length > 0 && aSort[0].col == i) {
              ...
              var nextSort = aoColumns[i].asSorting[aSort[0].index + 1] ? aoColumns[i].asSorting[aSort[0].index + 1] : aoColumns[i].asSorting[0];
      ...
      $.extend(DataTable.ext.aTypes, [
        // Numeric data type
        function (data) {
          return data === '' || data === '-' || (!isNaN(parseFloat(data)) && isFinite(data)) ? 'numeric' : null;
        },

        // Dates (only those recognised by the browser's Date.parse)
        function (data) {
          var parsed = Date.parse(data);
          return (parsed !== null && !isNaN(parsed)) || (typeof data === 'string' && data.length === 0) ? 'date' : null;
        },

        // HTML
        function (data) {
          return typeof data === 'string' && data.indexOf('<') != -1 && data.indexOf('>') != -1 ? 'html' : null;
        },
      ]);
